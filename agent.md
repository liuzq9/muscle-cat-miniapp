...
当前前端支持两种数据模式：默认使用 `src/services/api.ts` 内的 mock 数据，设置 `VITE_USE_MOCK=false` 后通过 HTTP API 连接 NestJS 后端。小程序运行前必须先编译，微信开发者工具导入编译产物而不是源码目录。
...
编译完成后，将 `unpackage/dist/dev/mp-weixin` 导入微信开发者工具。不要导入 `muscle-cat-miniapp` 源码根目录，也不要直接运行 `src`。
...
│   ├── services/api.ts        # mock 数据与后端请求适配层
...
- 不要把后端、数据库或密钥写入小程序前端；前端只能通过 `src/services/api.ts` 访问 API。
- 不要删除或提交 `node_modules`、`.npm-cache`、`unpackage` 等生成物，除非任务明确要求。
...
- 需要新增页面时同步更新 `src/pages.json`；需要新增领域对象时先更新 `src/types/domain.ts`。
- 避免破坏 mock 模式；新接口应同时提供可用的 mock 分支或明确记录暂不支持 mock。
- 保留 `project.config.json` 的编译产物配置，微信工具目标目录为 `unpackage/dist/dev/mp-weixin/`。

## 代码规范

- 使用 Vue 3 `<script setup lang="ts">`，优先组合式 API 和明确的类型标注。
- 页面组件使用 PascalCase 之外的现有文件命名方式保持不变；变量、函数使用 camelCase，类型和接口使用 PascalCase。
- API 请求集中在 `src/services/api.ts`，页面不直接拼接重复的请求实现。
- 优先复用 `src/types/domain.ts` 的类型；避免 `any`，只有处理 uni-app 页面 options 等无类型对象时才局部使用。
- 样式使用 SCSS 和 `src/styles/tokens.scss` 变量；页面专属样式使用 `scoped`，嵌套不超过三层。
...
- `npm run build:mp-weixin` 能成功完成，生成 `unpackage/dist/dev/mp-weixin/app.json` 等产物。
