...
export const api = {
  ...
  bookings: (memberId?: string) => bookingApi.list(memberId),
  history: (memberId?: string) => attendanceApi.history(memberId),
  createBooking: (payload: Parameters<typeof bookingApi.create>[0]) => bookingApi.create(payload),
  cancelBooking: (id: string, memberId?: string) => bookingApi.cancel(id, memberId),
  rescheduleBooking: (
    id: string,
    payload: Parameters<typeof bookingApi.reschedule>[1],
    memberId?: string,
  ) => bookingApi.reschedule(id, payload, memberId),
